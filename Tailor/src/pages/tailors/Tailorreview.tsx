...
const TailorReview = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.pageBg, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", fontFamily: "'DM Sans',sans-serif", position: "relative", transition: "background 0.3s" }}>
      <style>{`
      ...
      <div className="review-card fade-up" style={{ width: "100%", maxWidth: "460px", background: t.cardBg, borderRadius: "24px", padding: "40px", boxShadow: "0 8px 48px rgba(139,115,85,0.12)", border: `1px solid ${t.cardBorder}`, position: "relative", zIndex: 1, transition: "background 0.3s" }}>
        {stage === "form" && (
          <>
            <div style={{ textAlign: "center", marginBottom: "28px" }}>
              <span style={{ fontSize: "36px", display: "block", marginBottom: "10px" }}>✂️</span>
              <h1 style={{ fontFamily: "'Lora',serif", fontSize: "24px", color: t.title, fontWeight: 600, margin: "0 0 4px" }}>Tailor Review</h1>
              <p style={{ color: t.sub, fontSize: "11px", letterSpacing: "2.5px", margin: 0 }}>SHARE YOUR EXPERIENCE</p>
              <div style={{ width: "36px", height: "2px", background: "linear-gradient(90deg,#8b7355,#c9a84c)", borderRadius: "2px", margin: "14px auto 0" }} />
            </div>

            <div style={{ marginBottom: "22px" }}>
              <label style={{ display: "block", fontSize: "11px", fontWeight: 600, color: t.label, marginBottom: "8px", letterSpacing: "1px", textTransform: "uppercase" }}>Tailor's Mobile Number</label>
              <input style={{ ...inp, borderColor: ui.tailorFound ? t.accent : ui.phoneError ? "#c0392b" : t.inputBorder }}
                ...
                onBlur={e => e.target.style.borderColor = ui.tailorFound ? t.accent : ui.phoneError ? "#c0392b" : t.inputBorder} />
              {ui.searching && <p style={{ color: t.hint, fontSize: "12px", marginTop: "6px" }}>Searching...</p>}
              {ui.tailorFound && <p style={{ color: t.found, fontSize: "13px", fontWeight: 600, marginTop: "6px" }}>✓ {form.tailorName}</p>}
              {ui.phoneError && <p style={{ color: "#c0392b", fontSize: "11px", marginTop: "4px" }}>{ui.phoneError}</p>}
            </div>

            <div style={{ marginBottom: "22px" }}>
              <label style={{ display: "block", fontSize: "11px", fontWeight: 600, color: t.label, marginBottom: "8px", letterSpacing: "1px", textTransform: "uppercase" }}>Rate Your Experience</label>
              <div style={{ display: "flex", justifyContent: "center", gap: "6px" }}>
                {[1,2,3,4,5].map(star => (
                  <button key={star} style={{ background: "none", border: "none", cursor: "pointer", padding: "2px", transition: "transform 0.15s" }}
            ...
            <div style={{ marginBottom: "22px" }}>
              <label style={{ display: "block", fontSize: "11px", fontWeight: 600, color: t.label, marginBottom: "8px", letterSpacing: "1px", textTransform: "uppercase" }}>Write Your Review</label>
              ...
              <p style={{ textAlign: "right", fontSize: "11px", color: t.charCount, marginTop: "4px" }}>{form.review.length}/300</p>
            </div>

            {ui.submitError && <p style={{ color: "#c0392b", fontSize: "12px", marginBottom: "12px" }}>{ui.submitError}</p>}

            <button style={{ width: "100%", padding: "14px", background: t.accent, color: "#fef3e2", border: "none", borderRadius: "12px", fontSize: "14px", fontWeight: 600, cursor: "pointer", transition: "background 0.2s", opacity: ui.loading ? 0.7 : 1 }}
        ...
        {stage === "success" && (
          <div className="fade-up" style={{ textAlign: "center" }}>
            ...
            <h2 style={{ fontFamily: "'Lora',serif", fontSize: "24px", color: t.title, fontWeight: 600, marginBottom: "8px" }}>Review Published!</h2>
            <p style={{ color: t.sub, fontSize: "11px", letterSpacing: "2.5px" }}>Thank you for reviewing</p>
            <p style={{ fontFamily: "'Lora',serif", fontSize: "18px", color: t.title, fontWeight: 600, margin: "4px 0 20px" }}>{form.tailorName}</p>
            <div style={{ display: "flex", justifyContent: "center", gap: "6px", marginBottom: "16px" }}>
              {[1,2,3,4,5].map(s2 => (
                <svg key={s2} width="28" height="28" viewBox="0 0 24 24" fill={s2 <= form.rating ? "#c9a84c" : "none"} stroke={s2 <= form.rating ? "#c9a84c" : "#d4c4a8"} strokeWidth="1.5">
                  <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
                </svg>
              ))}
            </div>
            <p style={{ color: t.sub, fontSize: "13px", fontStyle: "italic", margin: "0 0 24px", lineHeight: 1.6 }}>"{form.review}"</p>
            <button style={{ width: "100%", padding: "14px", background: t.accent, color: "#fef3e2", border: "none", borderRadius: "12px", fontSize: "14px", fontWeight: 600, cursor: "pointer", transition: "background 0.2s" }}
              onClick={reset}
              onMouseEnter={e => e.currentTarget.style.background = t.accentHover}
              onMouseLeave={e => e.currentTarget.style.background = t.accent}>
